...
class Wave2DSimulator:
    ...
    def __init__(
        ...
        if self.cfl_condition > 1.0 / np.sqrt(2):
            raise ValueError(
                f"CFL condition violated: {self.cfl_condition:.3f} > {1.0/np.sqrt(2):.3f}"
            )
    ...
    def simulate_step(
        ...
        laplacian[1:-1, 1:-1] = (
            u_current[2:, 1:-1]
            + u_current[:-2, 1:-1]  # d²u/dx²
            + u_current[1:-1, 2:]
            + u_current[1:-1, :-2]  # d²u/dy²
...
def test_wave_simulator():
    ...
    # Visualize result
    sim.visualize_wave(
        final_wave, source_x, source_y, title="Test Wave Simulation (t=100)"
    )
