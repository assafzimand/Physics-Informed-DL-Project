...
class WaveDatasetGenerator:
    ...
    def generate_dataset(
        self, num_samples: int, timesteps: int, dataset_name: str = "wave_dataset"
        ...
        wave_fields = np.zeros(
            (num_samples, self.grid_size, self.grid_size), dtype=np.float32
        )
        source_coords = np.zeros((num_samples, 2), dtype=np.int32)

        # Generate samples with progress bar
        for i in tqdm(
            range(num_samples), desc=f"Generating {dataset_name} (T={timesteps})"
        ):
        ...
        with h5py.File(filepath, "w") as f:
            ...
            for timestep, (wave_fields, source_coords) in datasets.items():
                ...
                group.create_dataset(
                    "coordinates", data=source_coords, compression="gzip"
                )
                ...
                print(
                    f"  - {group_name}: {len(wave_fields)} samples, "
    ...
    def load_datasets_hdf5(self, filepath: str) -> dict:
        ...
        with h5py.File(filepath, "r") as f:
            ...
            print(
                f"Metadata: grid_size={f.attrs['grid_size']}, "
...
def main():
    ...
    generator = WaveDatasetGenerator(grid_size=GRID_SIZE, wave_speed=WAVE_SPEED, dt=DT)
    ...
    for timestep in TIMESTEPS:
        ...
        wave_fields, source_coords = generator.generate_dataset(
            num_samples=NUM_SAMPLES, timesteps=timestep, dataset_name=f"T_{timestep}"
        )
        ...
        print(
            f"  - Wave amplitude range: [{wave_fields.min():.3f}, {wave_fields.max():.3f}]"
        )
