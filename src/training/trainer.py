...
class WaveTrainer:
    ...
    def _create_dataloaders(self):
        ...
        self.logger.info("📊 Dataset splits:")
    ...
    def _create_scheduler(self):
        ...
        if self.config.scheduler_type.lower() == "plateau":
            ...
            scheduler = optim.lr_scheduler.StepLR(
                self.optimizer, step_size=20, gamma=self.config.scheduler_factor
            )
    ...
    def train_epoch(self) -> Tuple[float, float]:
        ...
        train_pbar = tqdm(
            self.train_loader, desc=f"Epoch {self.current_epoch} [Train]", leave=False
        )
        ...
        for wave_fields, targets in train_pbar:
            ...
            train_pbar.set_postfix(
                {"Loss": f"{loss.item():.4f}", "Dist_Err": f"{distance_error:.1f}px"}
            )
    ...
    def validate_epoch(self) -> Tuple[float, float]:
        ...
        val_pbar = tqdm(
            self.val_loader, desc=f"Epoch {self.current_epoch} [Valid]", leave=False
        )
    ...
    def train(self):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            self.logger.info(
                f"  Train Loss: {train_loss:.6f}, Distance Error: {train_distance_error:.2f} pixels"
            )
            if val_loss > 0:
                self.logger.info(
                    f"  Val Loss: {val_loss:.6f}, Distance Error: {val_distance_error:.2f} pixels"
                )
