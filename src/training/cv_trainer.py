...
class CrossValidationTrainer:
    ...
    def __init__(self, config: TrainingConfig, k_folds: int = 5):
        ...
        self.experiment_name = getattr(
            config, "experiment_name", "cv_wave_source_localization"
        )
        self.run_name = getattr(
            config,
            "run_name",
            f"5fold_cv_{config.learning_rate}_{config.batch_size}_{config.optimizer}",
        )

        print("🔬 5-Fold Cross-Validation Trainer Initialized")
        print(f"📊 Folds: {k_folds}")
        print(
            f"🎯 Config: LR={config.learning_rate}, BS={config.batch_size}, OPT={config.optimizer}"
        )
    ...
    def run_cross_validation(self) -> Dict[str, Any]:
        ...
        with mlflow.start_run(run_name=self.run_name):
            ...
            for fold_idx, (train_indices, val_indices) in enumerate(
                ...
                mlflow.log_metrics(
                    {
                        ...
                        f"fold_{fold_idx + 1}_distance_error": fold_result[
                            "distance_error"
                        ],
                        f"fold_{fold_idx + 1}_training_time": fold_result[
                            "training_time"
                        ],
                    }
                )
                ...
                print(
                    f"✅ Fold {fold_idx + 1} completed: {fold_result['distance_error']:.2f}px, {fold_result['val_loss']:.4f} loss"
                )
    ...
    def print_cv_summary(self, stats: Dict[str, Any]):
        ...
        print(
            f"   Mean: {stats['distance_error_mean']:.3f} ± {stats['distance_error_std']:.3f} px"
        )
        ...
        print(
            f"   Per fold: {stats['training_time_mean']:.1f} ± {stats['training_time_std']:.1f} minutes"
        )

        print("\n🏆 ACADEMIC REPORTING:")
        print(
            f"   Distance Error: {stats['distance_error_mean']:.2f} ± {stats['distance_error_std']:.2f} px"
        )
        print(
            f"   Validation Loss: {stats['val_loss_mean']:.4f} ± {stats['val_loss_std']:.4f}"
        )
