...
def test_best_cv_model():
    ...
    best_model_path = (
        "experiments/cv_full/data/models/cv_full_5fold_75epochs_fold_2_best.pth"
    )
...
def test_with_dataset_samples(inference, dataset_path, num_samples=5):
    ...
    with h5py.File(dataset_path, "r") as f:
        ...
        for i, idx in enumerate(test_indices):
            ...
            plt.imshow(wave_field, cmap="RdBu_r", aspect="equal")
            plt.scatter(
                *true_coords, color="red", s=100, marker="x", linewidth=3, label="True"
            )
            plt.scatter(
                *pred_coords,
                color="yellow",
            ...
            plt.scatter(
                *true_coords, color="red", s=150, marker="x", linewidth=3, label="True"
            )
            plt.scatter(
                *pred_coords,
                color="yellow",
...
Δy = {abs(pred_coords[1]-true_coords[1]):.2f}"""
            ...
            print(
                f"  Sample {idx}: True=({true_coords[0]:.1f}, {true_coords[1]:.1f}), "
        ...
        print("   Expected (from CV): 1.663 px")
...
def test_with_synthetic_samples(inference, num_samples=3):
    ...
    for i in range(num_samples):
        ...
        wave_field = simulator.simulate_wave_propagation(
            source_pos=true_coords, total_time=500, max_timesteps=500
        )
        ...
        print(
            f"  Sample {i+1}: True=({true_coords[0]:.1f}, {true_coords[1]:.1f}), "
            f"Pred=({pred_coords[0]:.1f}, {pred_coords[1]:.1f}), Error={error:.2f} px"
        )
