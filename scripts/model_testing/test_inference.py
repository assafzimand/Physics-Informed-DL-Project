...
def create_test_sample(timesteps: int = 500) -> tuple:
    ...
    simulator = Wave2DSimulator(
        grid_size=GRID_SIZE, wave_speed=WAVE_SPEED, dt=DT, dx=DX
    )
    ...
    print("✅ Test sample generated successfully!")
...
def test_inference_pipeline():
    ...
    try:
        ...
        if distance_error < 5.0:
            print("✅ EXCELLENT: Error < 5 pixels")
        elif distance_error < 10.0:
            print("✅ GOOD: Error < 10 pixels")
        elif distance_error < 20.0:
            print("⚠️  FAIR: Error < 20 pixels")
        else:
            print("❌ POOR: Error > 20 pixels")
...
def create_combined_visualization(
    results_list, save_path="results/combined_inference_test.png"
):
    """
    ...
    fig.suptitle(
        "Wave Source Localization: 10 Test Samples", fontsize=16, fontweight="bold"
    )
...
def test_multiple_samples(num_samples: int = 10):
    ...
    try:
        ...
        for i in range(num_samples):
            ...
            print(
                f"   True: ({true_x}, {true_y}) | Pred: ({pred_x:.1f}, {pred_y:.1f}) | Error: {distance_error:.2f}px"
            )
        ...
        print(
            f"  - Samples with error < 5px: {sum(1 for e in errors if e < 5.0)}/{num_samples}"
        )
        print(
            f"  - Samples with error < 10px: {sum(1 for e in errors if e < 10.0)}/{num_samples}"
        )
