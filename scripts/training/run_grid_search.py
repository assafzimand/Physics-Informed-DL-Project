...
def generate_hyperparameter_combinations(grid_name: str) -> List[Dict[str, Any]]:
    ...
    if grid_name in HYPERPARAMETER_GRIDS:
        ...
        raise ValueError(
            f"Unknown grid: {grid_name}. Available: {list(HYPERPARAMETER_GRIDS.keys())}"
        )
...
def run_grid_search(
    ...
    for i, params in enumerate(combinations):
        ...
        try:
            ...
            final_val_distance_error = (
                history["val_distance_error"][-1]
                if history["val_distance_error"]
                else float("inf")
            )
    ...
    print(
        f"📊 Successful runs: {sum(1 for r in results if 'error' not in r)}/{total_combinations}"
    )
    ...
    if successful_results:
        ...
        print(
            f"   Final Distance Error: {best_result['final_val_distance_error']:.2f} pixels"
        )
        ...
        with open(best_config_path, "w") as f:
            ...
            f.write(
                f"# Distance Error: {best_result['final_val_distance_error']:.2f} pixels\n\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", default="T500", choices=["T250", "T500"], help="Dataset to use"
    )
    parser.add_argument(
        "--grid",
    ...
    parser.add_argument(
        "--experiment", default="grid_search", help="MLflow experiment name"
    )
    parser.add_argument(
        "--test", action="store_true", help="Run quick test instead of full grid search"
    )
