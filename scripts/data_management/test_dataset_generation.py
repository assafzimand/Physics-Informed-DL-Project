...
def visualize_dataset_samples(
    wave_fields, source_coords, timesteps, num_samples, generation_time
):
    ...
    for i, idx in enumerate(sample_indices):
        ...
        # Plot wave field
        im = ax.imshow(
            wave_field, cmap="RdBu_r", origin="lower", vmin=wave_min, vmax=wave_max
        )
        ax.plot(
            source_x,
            source_y,
    ...
    print(
        f"   Source coordinates range: X[{source_coords[:, 0].min()}-{source_coords[:, 0].max()}], Y[{source_coords[:, 1].min()}-{source_coords[:, 1].max()}]"
    )
...
def main():
    ...
    # Generate dataset
    wave_fields, source_coords, generation_time = generate_dataset_samples(
        timesteps, num_samples
    )
