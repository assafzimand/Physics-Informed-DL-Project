...
def animated_test():
    ...
    u_current, u_previous = sim.create_initial_conditions(
        source_x, source_y, **init_params
    )
    ...
    im = ax.imshow(
        wave_data, cmap="RdBu_r", origin="lower", vmin=vmin, vmax=vmax, animated=True
    )
    ...
    anim = animation.FuncAnimation(
        fig, animate, frames=len(wave_history), interval=30, blit=True, repeat=True
    )
