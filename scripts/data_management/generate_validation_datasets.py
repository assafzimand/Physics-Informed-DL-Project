...
def generate_validation_dataset_samples(
    timesteps, num_samples, dataset_name, validation_seed
):
    ...
    # Initialize storage
    wave_fields = np.zeros(
        (num_samples, config.GRID_SIZE, config.GRID_SIZE), dtype=np.float32
    )
...
def save_validation_dataset_hdf5(
    wave_fields, source_coords, timesteps, generation_time, filename, validation_seed
):
    ...
    with h5py.File(filepath, "w") as f:
        ...
        f.create_dataset(
            "wave_fields", data=wave_fields, compression="gzip", compression_opts=6
        )
        f.create_dataset(
            "source_coords", data=source_coords, compression="gzip", compression_opts=6
        )
...
def main():
    ...
    print("📋 Validation Generation Plan:")
    print(
        f"   Dataset 1: {num_samples} samples, T={timesteps_list[0]} (seed={validation_seeds[250]})"
    )
    print(
        f"   Dataset 2: {num_samples} samples, T={timesteps_list[1]} (seed={validation_seeds[500]})"
    )
    ...
    wave_fields_250, source_coords_250, gen_time_250 = (
        generate_validation_dataset_samples(
            timesteps_list[0],
            num_samples,
            f"Validation T={timesteps_list[0]}",
    ...
    wave_fields_500, source_coords_500, gen_time_500 = (
        generate_validation_dataset_samples(
            timesteps_list[1],
            num_samples,
            f"Validation T={timesteps_list[1]}",
            validation_seeds[500],
        )
    )
    ...
    print("\n📁 Files saved in: data/")
    print("🎯 Next step: Use scripts/extra_validation.py to test models")
    print("=" * 60)
