...
def plot_random_samples():
    ...
    for i, idx in enumerate(indices_250):
        ...
        im = ax.imshow(
            wave, cmap="RdBu_r", origin="lower", vmin=global_min, vmax=global_max
        )
        ax.plot(
            source_x,
            source_y,
        ...
        print(
            f"  Sample {idx}: source=({source_x:3d}, {source_y:3d}), range=[{wave.min():.3f}, {wave.max():.3f}]"
        )
    ...
    # Set main title
    plt.suptitle(
        "Random Dataset Samples: Wave Patterns and Source Locations",
        fontsize=16,
        y=0.98,
    )
