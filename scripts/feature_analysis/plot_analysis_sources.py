...
def plot_source_locations(dataset_file, output_dir):
    ...
    # Add sample numbers
    for i, (coord, cat, orig_idx) in enumerate(
        zip(coordinates, categories, original_indices)
    ):
        ax.annotate(
            f"{i}",
            (coord[0], coord[1]),
    ...
    ax.set_title(
        "Analysis Dataset: Source Location Distribution\n"
        + "20 Samples for Feature Visualization",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
