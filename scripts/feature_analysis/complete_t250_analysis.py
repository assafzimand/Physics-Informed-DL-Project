...
def select_t250_samples(n_samples=20):
    ...
    def categorize_source_location(coords, grid_size=128):
        ...
        if (
            ...
            or (x < margin and y > grid_size - margin)
            or (x > grid_size - margin and y < margin)
            or (x > grid_size - margin and y > grid_size - margin)
        ):
            return "corner"
        # Edge check
        elif (
            x < margin or x > grid_size - margin or y < margin or y > grid_size - margin
        ):
            return "edge"
        # Center
        else:
    ...
    for i in range(total_samples):
        ...
        samples_by_category[category].append(
            {"index": i, "coords": coords_np.tolist(), "category": category}
        )
    ...
    if len(remaining_samples) >= needed_random:
        random_indices = np.random.choice(
            len(remaining_samples), needed_random, replace=False
        )
...
def create_t250_dataset():
    ...
    with h5py.File(output_file, "w") as f:
        ...
        coordinates = f.create_dataset(
            "coordinates", shape=(n_samples, 2), dtype=np.float32
        )

        original_indices = f.create_dataset(
            "original_indices", shape=(n_samples,), dtype=np.int32
        )

        categories = f.create_dataset(
            "categories", shape=(n_samples,), dtype=h5py.string_dtype()
        )
...
def extract_t250_features():
    ...
    print("✅ T=250 feature extraction complete!")
    return True
...
def create_t250_comprehensive_plots():
    ...
    print("✅ All T=250 comprehensive plots created!")
...
def create_t250_comprehensive_plot(sample_id, features, sample_info, wave_field):
    ...
    error = np.linalg.norm(
        [pred_coords[0] - true_coords[0], pred_coords[1] - true_coords[1]]
    )
    fig.suptitle(
        f"T=250 Feature Analysis: {sample_id}\n"
