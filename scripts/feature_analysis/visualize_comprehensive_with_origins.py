...
def create_comprehensive_visualization(sample_id, save_plot=True):
    ...
    features, sample_info = load_sample_features(
        sample_id, "experiments/feature_analysis/activations"
    )
    ...
    fig.suptitle(
        f"Comprehensive Feature Analysis: {sample_id}\n"
        ...
        for feat_idx in range(9):
            ...
            if feat_idx < len(feature_maps):
                ...
                peak_y, peak_x = np.unravel_index(
                    np.argmax(feature_map), feature_map.shape
                )
                ...
                if feat_idx == 4:  # Middle feature - add stage title
                    ...
                    ax.set_title(
                        f"F#{filter_idx} @({peak_x},{peak_y})→({orig_x},{orig_y})",
                        fontweight="bold",
                        fontsize=9,
                    )
            else:
        ...
        if stage_key in features:
            ...
            for feat_idx in range(9):
                ...
                if feat_idx < len(feature_maps):
                    ...
                    # Map to original image coordinates
                    orig_x, orig_y = map_to_original_coordinates(
                        peak_x, peak_y, stage=stage
                    )
    ...
    if save_plot:
        # Save plot
        output_dir = Path(
            "experiments/feature_analysis/plots/comprehensive_with_origins"
        )
...
def main():
    ...
    print(
        "🎨 Creating comprehensive feature visualization with original coordinate mapping..."
    )
    ...
    print(
        "📁 Check plots in: experiments/feature_analysis/plots/comprehensive_with_origins/"
    )
