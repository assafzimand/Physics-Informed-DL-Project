...
def create_comprehensive_visualization(sample_id, save_plot=True):
    ...
    for stage in range(1, 5):
        ...
        if stage_key in features:
            ...
            for feat_idx in range(9):
                col = (
                    feat_idx + 1
                )  # Columns 1-9 (we use 1-10 but skip column 0 for stages 1-4)
    ...
    if save_plot:
        # Save plot
        output_dir = Path(
            "experiments/feature_analysis/plots/comprehensive_with_coords"
        )
...
def main():
    ...
    print(
        "📁 Check plots in: experiments/feature_analysis/plots/comprehensive_with_coords/"
    )
