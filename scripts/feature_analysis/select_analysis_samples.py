...
def categorize_source_location(coords, grid_size=128):
    ...
    # Edge check
    elif x < margin or x > grid_size - margin or y < margin or y > grid_size - margin:
        return "edge"
...
def select_diverse_samples(dataset_path, n_samples=20):
    ...
    if len(edge_samples) >= 8:
        ...
        if len(center_samples) >= remaining:
            extra_centers = np.random.choice(
                len(center_samples), remaining, replace=False
            )
    ...
    if len(center_samples) >= needed_centers:
        # Avoid duplicates
        used_center_indices = [
            s["index"] for s in selected_samples if s["category"] == "center"
        ]
        available_centers = [
            s for s in center_samples if s["index"] not in used_center_indices
        ]

        if len(available_centers) >= needed_centers:
            selected_center_indices = np.random.choice(
                len(available_centers), needed_centers, replace=False
            )
...
def main():
    ...
    # Save results
    metadata_file, raw_data_dir = save_sample_data(
        selected_samples, dataset_path, output_dir
    )

    print("\n🎉 Sample selection complete!")
    print(f"📁 Results saved to: {output_dir}")
    print("📊 Next step: Create feature extraction script")
