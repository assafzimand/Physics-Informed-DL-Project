...
class SimpleFeatureExtractor:
    ...
    def extract_activations(self, wave_input):
        ...
        for i, stage_name in enumerate(stage_names):
            if stage_name in self.model.activations:
                ...
                activation = self.model.activations[stage_name].squeeze(
                    0
                )  # Remove batch dim
    ...
    def process_sample(self, sample_path, n_features=9):
        ...
        print(
            f"   Processing sample {sample_path.name}: {category} source at {coordinates}"
        )
...
def process_all_samples(model_path, samples_dir, output_dir, n_features=9):
    ...
    print(
        f"📊 Extracted {n_features} top features per stage for {len(all_results)} samples"
    )
...
def main():
    ...
    print("\n🎉 Feature extraction complete!")
    print("📊 Next step: Create visualization plots")
