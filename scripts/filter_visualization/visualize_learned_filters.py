    ...
    for stage_num, stage_name, stage_desc in stage_info:
        if hasattr(model, stage_name):
            ...
            for block_idx in range(len(stage)):
                ...
                # Conv1 in this block
                if hasattr(block, "wave_feature_conv1") and isinstance(
                    block.wave_feature_conv1, torch.nn.Conv2d
                ):
                    conv_layers.append(
                        {
                ...
                # Conv2 in this block
                if hasattr(block, "wave_feature_conv2") and isinstance(
                    block.wave_feature_conv2, torch.nn.Conv2d
                ):
                    conv_layers.append(
                        {
                            "layer_id": layer_counter,
...
def create_filter_weight_visualization(
    filter_weights, filter_indices, layer_info, save_path
):
    ...
    print(
        f"📊 Filter info: {in_channels} input channels, {filter_h}x{filter_w} kernel size"
    )
    ...
    if in_channels == 1:
        ...
        print(
            f"📊 Raw weight range: [{global_min:.4f}, {global_max:.4f}] - Using raw values (no normalization)"
        )

        for i, (filter_weight, filter_idx) in enumerate(
            zip(filter_weights, filter_indices)
        ):
            row, col = i // cols, i % cols
            ax = axes[row, col]
        ...
        print(
            f"📊 Raw averaged weight range: [{global_min:.4f}, {global_max:.4f}] - Using raw values (no normalization)"
        )
        ...
        for i, (filter_weight, filter_idx) in enumerate(
            ...
            f_min, f_max = weight_2d.min(), weight_2d.max()
            ax.set_title(
                f"Filter {filter_idx} (avg {in_channels}ch)\nRange: [{f_min:.4f}, {f_max:.4f}]",
                fontsize=10,
                fontweight="bold",
            )
            ax.set_xticks([])
    ...
    # Main title
    fig.suptitle(
        f'Learned Filter Weights: {layer_info["name"]}\n'
...
def main():
    ...
    for layer in conv_layers:
        print(
            f"   Layer {layer['layer_id']:2d}: {layer['name']} ({layer['filters']} filters)"
        )
    ...
    selected_layer = next(
        layer for layer in conv_layers if layer["layer_id"] == selected_layer_id
    )
    ...
    while True:
        try:
            num_filters_to_show = int(
                input(f"Enter number of filters to visualize (1-{max_filters}): ")
            )
    ...
    create_filter_weight_visualization(
        filter_weights, filter_indices, selected_layer, save_path
    )
