...
def create_enhanced_filter_visualization(
    ...
    print(
        f"📊 Enhanced visualization for {num_filters} filters ({filter_h}x{filter_w})"
    )
    ...
    fig.suptitle(
        f'Layer {layer_info["layer_id"]}: {layer_info["name"]} - Enhanced Filter Visualization',
        fontsize=16,
        fontweight="bold",
    )
...
def create_large_scale_visualization(
    filter_weights, filter_indices, layer_info, save_path
):
    ...
    for i in range(num_filters):
        ...
        upsampled = zoom(
            filter_weight, 4, order=3
        )  # 4x upsampling with cubic interpolation
        ...
        im = ax.imshow(
            enhanced, cmap="RdBu_r", interpolation="bilinear", vmin=-2, vmax=2
        )
    ...
    plt.suptitle(
        f'Layer {layer_info["layer_id"]}: {layer_info["name"]}\n'
...
def analyze_filter_patterns(filter_weights, filter_indices):
    ...
    for i, (filter_weight, filter_idx) in enumerate(
        ...
        edge_vals = np.concatenate(
            [weight_2d[0, :], weight_2d[-1, :], weight_2d[:, 0], weight_2d[:, -1]]
        )
        ...
        print(
            f"   Filter {filter_idx:2d}: {pattern_type:15s} | "
...
def main():
    ...
    create_enhanced_filter_visualization(
        filter_weights, filter_indices, layer_info, enhanced_path
    )

    large_scale_path = output_dir / "layer_0_large_scale_filters.png"
    create_large_scale_visualization(
        filter_weights, filter_indices, layer_info, large_scale_path
    )
