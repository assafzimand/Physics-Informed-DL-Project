...
def analyze_layer0_filters(model):
    ...
    if hasattr(model, "wave_input_processor") and len(model.wave_input_processor) > 0:
        ...
        if isinstance(conv_layer, torch.nn.Conv2d):
            weights = (
                conv_layer.weight.data.cpu()
            )  # Shape: [out_channels, in_channels, H, W]

            print("📊 Layer 0 Conv2d Details:")
            ...
            for i in range(min(5, weights.shape[0])):
                ...
                print(
                    f"   Filter {i}: min={filter_weight.min():.4f}, max={filter_weight.max():.4f}, "
...
def visualize_raw_filters(weights, num_filters=9):
    ...
    plt.suptitle(
        "Layer 0 Filter Weights - Raw Analysis", fontsize=16, fontweight="bold"
    )
    plt.tight_layout()
...
def compare_with_random_filters():
    ...
    print("📊 Random Filter Statistics:")
...
def main():
    ...
    print("📁 Results saved in: experiments/filter_visualization/debug/")
