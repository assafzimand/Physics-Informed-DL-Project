#!/usr/bin/env python3
"""
Failure Analysis: T250 Validation Prediction Scatter Heatmap by Error Percentile

Reads a T250 validation run folder (e.g., experiments/t250_cv_full/extra_validation/validation_20250723_2352),
...
Output is saved under experiments/t250_cv_full/analysis/failure_analysis/.
"""
...
def main():
    # Inputs - T250 specific paths
    validation_dir = Path("experiments/t250_cv_full/extra_validation/validation_20250723_2352")
    t250_root = Path("experiments/t250_cv_full")
    analysis_dir = t250_root / "analysis" / "failure_analysis"

    summary = read_validation_summary(validation_dir)
    dataset_name = summary.get("dataset_info", {}).get("dataset_path", "data/wave_dataset_T250_validation.h5")
    dataset_path = Path(dataset_name)
    model_name = summary.get("model_info", {}).get("model_name", "")
    if not model_name:
        raise ValueError("model_name missing in validation summary")

    # Resolve training stats from TRAIN file (T250)
