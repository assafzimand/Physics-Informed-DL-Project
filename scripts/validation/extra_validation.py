        ...
        try:
            ...
            for line in content.split("\n"):
                if line.startswith("Individual Fold Errors:"):
                    ...
                    print(
                        f"✅ Best fold determined from CV results: Fold {best_fold} ({min_error:.3f} px)"
                    )
                    break
...
def load_validation_dataset(T_value):
    ...
    if not Path(dataset_path).exists():
        ...
        print("💡 Run: python scripts/generate_validation_datasets.py")
        return None
...
def create_validation_plots(
    distance_errors, x_errors, y_errors, predictions, ground_truths, T_value, stats_dict
):
    ...
    fig.suptitle(
        f"T={T_value} Model Extra Validation Analysis", fontsize=16, fontweight="bold"
    )

    # Plot 1: Distance Error Histogram
    axes[0, 0].hist(
        distance_errors, bins=30, alpha=0.7, color="skyblue", edgecolor="black"
    )
    axes[0, 0].axvline(
    ...
    axes[1, 0].scatter(
        ground_truths[:, 0], predictions[:, 0], alpha=0.6, s=20, label="X coordinate"
    )
    axes[1, 0].scatter(
        ground_truths[:, 1], predictions[:, 1], alpha=0.6, s=20, label="Y coordinate"
    )
...
def save_validation_results(T_value, stats_dict, model_info, fig):
    ...
    with open(report_file, "w") as f:
        ...
        f.write(
            f"Extra validation of T={T_value} best model on dedicated 500-sample validation dataset.\n\n"
        )
        ...
        f.write(
            f"- **Mean Error**: {stats_dict['distance_error']['mean']:.3f} ± {stats_dict['distance_error']['std']:.3f} px\n"
        )
        f.write(
            f"- **Median Error**: {stats_dict['distance_error']['median']:.3f} px\n"
        )
        f.write(
            f"- **Min/Max Error**: {stats_dict['distance_error']['min']:.3f} / {stats_dict['distance_error']['max']:.3f} px\n"
        )
        f.write(
            f"- **95% Confidence Interval**: [{stats_dict['confidence_interval_95']['lower']:.3f}, {stats_dict['confidence_interval_95']['upper']:.3f}] px\n\n"
        )
        ...
        if "normality_test" in stats_dict:
            ...
            f.write(
                f"- **Error Distribution Normal**: {normal_text} (p={stats_dict['normality_test']['p_value']:.4f})\n"
            )

        f.write(
            f"- **X Error Bias**: {stats_dict['x_error']['mean']:.4f} ± {stats_dict['x_error']['std']:.4f}\n"
        )
        f.write(
            f"- **Y Error Bias**: {stats_dict['y_error']['mean']:.4f} ± {stats_dict['y_error']['std']:.4f}\n"
        )

        f.write("\n## 📁 Generated Files\n")
        f.write(f"- `validation_results_{timestamp}.json`: Complete statistical data\n")
        f.write(
            f"- `validation_analysis_{timestamp}.png`: Comprehensive analysis plots\n"
        )
        ...
        f.write(
            f"The T={T_value} model demonstrates {conclusion.lower()} with a mean distance error of {mean_error:.3f} pixels."
        )
...
def main():
    ...
    for exp in experiments:
        ...
        stats_dict, distance_errors, x_errors, y_errors = calculate_error_statistics(
            predictions, ground_truths
        )

        # Create plots
        fig = create_validation_plots(
            distance_errors,
            x_errors,
            y_errors,
            predictions,
        ...
        print(
            f"✅ T={T_value} validation complete: {stats_dict['distance_error']['mean']:.3f} ± {stats_dict['distance_error']['std']:.3f} px"
        )
